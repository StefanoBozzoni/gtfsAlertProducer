...
public interface RoutesRepository extends JpaRepository<Routes, Integer> {
    ...
    @Query(value = "select distinct s.shape_pt_lat as lat,s.shape_pt_lon as lon, s.shape_id as name ,s.shape_pt_sequence, '#FFFF00' as color , 'x' as note from {h-schema}shapes s where shape_id in (select shape_id from {h-schema}trips where route_id =:route_id and coalesce(direction_id,b'0') = b'0') order by s.shape_id, s.shape_pt_sequence", nativeQuery = true)   
